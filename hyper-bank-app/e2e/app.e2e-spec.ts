...
describe('Starting tests for hyper-bank-app', function() {
  ...
  it('website title should be hyper-bank-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('hyper-bank-app');
    })
  });

  it('network-name should be hyper-bank@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('hyper-bank@0.0.1.bna');
    });
  });

  it('navbar-brand should be hyper-bank-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('hyper-bank-app');
    });
  });
